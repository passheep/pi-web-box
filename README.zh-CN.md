...
- **多标签会话**：最多 6 个标签，复用同一个 Pi Web 服务，支持拖拽排序、中键关闭及 Ctrl+T / Ctrl+W / Ctrl+Tab。每个打开的标签使用不同彩点，会话运行时显示环形动画，关闭标签不主动终止会话。
- **本地消息中心**：右上角查看历史通知与未读角标；倒序每页 30 条，滚动到底加载下一页，结束提示随列表滚动。数据库位于 `%APPDATA%/Pi Web Box/notifications.sqlite`，仅记录 Box 页面实际收到的消息，断线或退出期间不补录。
- **发布日志**：[v0.7.2 更新说明](./release-notes/v0.7.2.md)。

- **标题栏自动跟随主题**：三个窗口都使用自绘标题栏（不依赖 Windows 原生标题栏），颜色直接取自 Pi Web 顶部工具栏的实际背景色，窗口与页面完全连成一体、没有分割线。Pi Web 有浅色、深色、雾青、蔷薇、松夜五种外观，切换后标题栏会自动同步，无需手动刷新。
...
GitHub Release 当前提供 Windows x64 安装包；便携版可按下方命令自行构建：
...
```text
dist/Pi Web Box Setup-0.7.2.exe       # 安装包
```
...
快捷方式目标始终是本次启动的 EXE，因此移动便携版后，只需从新位置启动一次，快捷方式就会自动更新。默认关闭窗口会最小化至托盘；从托盘退出应用或关闭该设置后退出，才会清理由应用本次启动的 Pi Web 服务；如果检测到已有 Pi Web 服务占用默认端口，则会复用它，不会停止外部服务。
...
标题栏会跟随 Pi Web 的当前主题自动上色，支持浅色、深色、雾青、蔷薇、松夜五种外观。主窗口右上角提供最小化、最大化、关闭；设置与统计窗口只提供关闭。关于面板、设置窗口、右下角悬浮球与滚动条的配色同样跟随主题，深浅色背景下都会自动切换到对应版本的 Logo。
